#!/usr/bin/env python3
"""
Prepare train / val / test JSONL splits for AdaLoRA fine-tuning.

Split rules are identical to LoRA — data is shared from dataset_full/.
...
Usage
-----
    python prepare_adalora_data.py --dataset-size 1k
    python prepare_adalora_data.py --dataset-size 10k
    python prepare_adalora_data.py --dataset-size 1k \\
        --data-dir /path/to/dataset_full --out-dir /path/to/finetune_AdaLoRA/data
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Prepare JSONL splits for AdaLoRA fine-tuning.",
...
def main() -> None:
    ...
    if args.dataset_size == "1k":
        train = load_files([fp(i) for i in range(2, 10)])
        val = load_files([fp(10)])
        test = anchor

    else:  # 10k
        train = load_files([fp(i) for i in range(2, 82)])
        val = load_files([fp(i) for i in range(82, 92)])
        test_rest = load_files([fp(i) for i in range(92, 101)])
        test = anchor + test_rest
