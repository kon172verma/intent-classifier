#!/usr/bin/env python3
"""
Phase 3 – Plot LoRA experiment results.

Reads training and evaluation reports to produce three figures:

  lora_accuracy_comparison.png  — bar chart of accuracy by (model, config)
  lora_training_curves.png      — loss + val-accuracy curves per run
  lora_memory_comparison.png    — peak inference memory by run
...
# Colours per config; markers per model for scatter/line contexts.
...
def plot_accuracy_comparison(
    ...
    fig, ax = plt.subplots(figsize=(max(8, n_models * 2.5), 5), constrained_layout=True)
    ...
    ax.set_title(
        f"LoRA Accuracy by Model & Config  ({split_label})",
        fontsize=12,
        fontweight="bold",
    )

    out_path = out_dir / "lora_accuracy_comparison.png"
...
def plot_training_curves(train_reports: list[dict], out_dir: Path) -> None:
    ...
    reports = sorted(
        train_reports, key=lambda r: f"{r['model_key']}_{r['lora_config']}"
    )
    n = len(reports)
    ...
    for ax, report in zip(axes[0], reports):
        ...
        train_steps = [
            h["step"] for h in history if "loss" in h and "eval_loss" not in h
        ]
        train_losses = [
            h["loss"] for h in history if "loss" in h and "eval_loss" not in h
        ]
    ...
    out_path = out_dir / "lora_training_curves.png"
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved → {out_path}")
...
def plot_memory_comparison(eval_reports: list[dict], out_dir: Path) -> None:
    ...
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.set_title("Peak Inference Memory by Run", fontsize=12, fontweight="bold")
    ...
    out_path = out_dir / "lora_memory_comparison.png"
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved → {out_path}")
...
def main() -> None:
    ...
    reports_dir = (
        args.test_reports_dir if args.split == "test" else args.val_reports_dir
    )
