...
1. HuggingFace Hub (default):
     PeftModel.from_pretrained(base, HF_EXPERIMENTS_REPO, subfolder="v1.0/{model}_LoRA_{config}_{size}_{timestamp}")
...
def validate_main(
    ...
    if not args.local:
        ...
        if timestamp is None:
            raise ValueError(
                f"No experiment found in EXPERIMENTS.jsonl for {technique}/{run_tag} "
    ...
    if not args.local and hf_sub:
        uploaded_path = upload_report_to_hf(
            ...
            technique=technique,
            report_group=out_dir.name,
            commit_message=f"Add {technique} {args.split} report: {run_tag} [{ts}]",
        )
        if uploaded_path:
