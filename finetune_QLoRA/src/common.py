#!/usr/bin/env python3
"""
Shared utilities for finetune_QLoRA.

Contains the model registry, QLoRA config definitions, prompt helpers,
...
# ── QLoRA / LoRA adapter configs ──────────────────────────────────────────────
# Same adapter hyperparameters as finetune_LoRA so results are directly
# comparable.  The only difference between LoRA and QLoRA is that the base
# model weights are stored in NF4 (4-bit) during training, cutting VRAM ~50%.
#
...
# ── System prompt ─────────────────────────────────────────────────────────────
# Kept identical to baseline_evaluation and finetune_LoRA so comparisons are fair.
SYSTEM_PROMPT: str = (
