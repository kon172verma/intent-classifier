...
This is the "merge + unload" step of the release pipeline, kept in
intent-classifier since making a release (merge/unload/publish) is this
repo's responsibility. The intent-classifier-inference repo only ever reads
the resulting merged models — it no longer performs merges itself.

Output feeds the GGUF (llama.cpp) and ONNX export tooling in
intent-classifier-inference, and/or release.py's push-to-HF_RELEASE_REPO step.
...
-----
    ...
    python scripts/merge_models.py --technique LoRA \\
...
DEFAULT_OUTPUT_DIR = _REPO_ROOT / "release_staging"
...
def merge_run(
    ...
    output_dir = output_root / f"intent-classifier-{run_tag}_merged"
    ...
    print(f"[ok] Completed: {output_dir.name}")
